// Component - Profile Analytics Section
// Displays learning analytics data from Learning Analytics microservice

import React, { useState, useEffect } from 'react';
import { useAuth } from '../context/AuthContext';
import { getEmployeeDashboard } from '../services/employeeService';

function ProfileAnalytics({ employeeId }) {
  const { user } = useAuth();
  ...
  useEffect(() => {
    const fetchAnalytics = async () => {
      ...
      try {
        ...
        const analytics = response?.dashboard || response?.response?.dashboard || response;
        setAnalyticsData(analytics);
      } catch (err) {
        console.error('[ProfileAnalytics] Error fetching analytics:', err);
        if (err.response?.status === 403) {
          setError('Your profile must be approved by HR to view analytics.');
        } else {
  ...
  const handleViewFullAnalytics = () => {
    // Redirect to Learning Analytics microservice frontend
    alert('Redirecting to LEARNING ANALYTICS');
    // TODO: When Learning Analytics frontend is available, redirect to it
    // const analyticsUrl = process.env.REACT_APP_LEARNING_ANALYTICS_URL || 'https://learning-analytics-production.up.railway.app';
    // window.open(`${analyticsUrl}/analytics?employee_id=${employeeId}`, '_blank');
  };
  ...
  const progressSummary = analyticsData?.progress_summary || {};
  const recentActivity = analyticsData?.recent_activity || [];
  const upcomingDeadlines = analyticsData?.upcoming_deadlines || [];
        ...
        {Object.keys(progressSummary).length === 0 && 
         ...
         achievements.length === 0 && (
          <p className="text-sm mb-4" style={{ color: 'var(--text-secondary)' }}>
            No analytics data available yet. Your learning progress will appear here.
          </p>
        )}
